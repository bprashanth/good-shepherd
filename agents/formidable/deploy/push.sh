#!/usr/bin/env bash
# Build + push both images, push codex credentials to Secrets Manager,
...
echo "=== Push codex credentials to Secrets Manager ==="
...
docker build --build-arg "CODEX_VERSION=${CODEX_VERSION}" -f "$SERVER_DIR/Dockerfile.worker" -t "$WORKER_IMAGE" "$SERVER_DIR"
...
HTTP_ENV="Variables={JOBS_BUCKET=${JOBS_BUCKET},S3_PREFIX=${S3_PREFIX},DYNAMO_TABLE=${DYNAMO_TABLE},ECS_CLUSTER=${ECS_CLUSTER},FARGATE_TASK=${FARGATE_TASK_DEF},ECS_SG_NAME=${ECS_SG_NAME},AWS_LWA_REMOVE_BASE_PATH=/prod}"
