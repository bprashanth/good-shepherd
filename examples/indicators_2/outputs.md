...
This document describes the artifacts emitted by `examples/indicators_2/`.

The stage mirrors `examples/indicators/`, but the inputs come from the completed Osuri restoration study rather than upload-time forms and trace mapping.
...
Wizard UI matching `examples/indicators` as closely as possible, without depending on trace-mapping workflow.

### `output/indicator_field_mapping.html`

Compatibility artifact retained so the sibling output shape stays close to `examples/indicators`, even though this stage does not rely on active trace mapping.

## Assumptions

- the compute server is reused from `examples/indicators/scripts/compute_server.py`
