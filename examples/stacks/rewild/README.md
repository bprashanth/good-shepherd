...
* Entrypoint: `examples/stacks/rewild/web/` -> this has cards for all the following 
* Site selection: `examples/stacks/rewild/setup_wizard/`
* Site Assesment: `examples/stacks/rewild/pwa/`
* Plantwise: `examples/stacks/rewild/plantwise/`
* Site Monitoring: `examples/stacks/rewild/pwa/`
* Indicators: `examples/stacks/rewild/indicators/`
...
There is an aoi in `examples/stacks/rewild/setup_wizard/input/`, see `examples/stacks/rewild/setup_wizard/input/README.md`. We need to show that outline. (hull around monitoring points from `anr_sites.json`). This simulates the kml. 

We need to add 1. alpha earth  embeddings for that hull, 2. the tif files from  `examples/stacks/research/alienwise/output`. 
...
The input of this site is exactly one monitoring plot from the output of site selection - show it in a similar style to the pwa code in `examples/stacks/research/pwa` - if need be copy that over and instrument it. But the idea is the user goes  to that plot, and submits a picture from each subplot. The picture will be one of lantana - they will upload it via file upload through the same file pwa interface plus button, just like in `examples/stacks/research/pwa`. The only difference is the app will show an advisory saying something like this is lantana, with some simple removal instructions. 
...
This will have a simple 2 panel display. In left panel a list of portrait images  from input/ folder. On right panel, the set of indicators from the `examples/stacks/rewild/indicators/input/` directory, most importantly the similarity score. These idicators should keep changing with the "next" button which also  changes the image. The idea is the user has taken some form data and photomonitoring images, and they use these two to measure the progress of their restoration. So as they click next they would see the indicator going up, and the image growing lusher.  
